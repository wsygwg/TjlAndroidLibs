...
import cn.com.hiss.www.sharephoto.util.AlbumHelper;
import cn.com.hiss.www.sharephoto.util.Bimp;
import cn.com.hiss.www.sharephoto.util.ImageBucket;
...
public class AlbumActivity extends Activity {
    ...
    // 返回按钮监听
    private class AlbumsListener implements OnClickListener {
        public void onClick(View v) {
    ...
    // 初始化，给一些对象赋值
    private void init() {
        Bimp.refreshPicList(getApplicationContext(), false);
        albums = (Button) findViewById(Res.getWidgetID("go2albums"));
        cancel = (Button) findViewById(Res.getWidgetID("cancel"));
        cancel.setOnClickListener(new CancelListener());
        albums.setOnClickListener(new AlbumsListener());
    ...
    private void initListener() {
        gridImageAdapter.setOnItemClickListener(new AlbumGridViewAdapter.OnItemClickListener() {
            ...
            public void onItemClick(final ToggleButton toggleButton, int position, boolean isChecked, Button chooseBt) {
                if (Bimp.TEMP_PIC_STORAGE().size() >= Bimp.GET_MAX_PIC_NUMBER()) {
                ...
                if (isChecked) {
                    ...
                    Log.e(TAG, "add " + dataList.get(position).toString());
                    okButton.setText(finishButtonText());
                } else {
                    Bimp.TEMP_PIC_STORAGE().remove(dataList.get(position));
                    Log.e(TAG, "remove " + dataList.get(position).toString());
                    chooseBt.setVisibility(View.GONE);
                    okButton.setText(finishButtonText());
                }
                isShowOkBt();
    ...
    private boolean removeOneData(ImageItem imageItem) {
        if (Bimp.TEMP_PIC_STORAGE().contains(imageItem)) {
            Bimp.TEMP_PIC_STORAGE().remove(imageItem);
            okButton.setText(finishButtonText());
            return true;
        }
        return false;
