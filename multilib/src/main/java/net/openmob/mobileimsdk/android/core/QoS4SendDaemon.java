...
public class QoS4SendDaemon {
    ...
    public static final int QOS_TRY_COUNT = 3;//重发3次
        ...
        this.runnable = new Runnable() {
            public void run() {
                ...
                if (!QoS4SendDaemon.this._excuting) {
                    new AsyncTask<Object, ArrayList<Protocal>, ArrayList<Protocal>>() {
                        ...
                        protected ArrayList<Protocal> doInBackground(Object[] params) {
                            ...
                            try {
                                if (ClientCoreSDK.DEBUG) {
                                    Log.e(QoS4SendDaemon.TAG, "【IMCORE】【QoS】=========== 消息发送质量保证线程运行中" + ", 当前需要处理的列表长度为" + QoS4SendDaemon.this.sentMessages.size() + "...");
                                }
                                ...
                                for (String key : QoS4SendDaemon.this.sentMessages.keySet()) {
                                    ...
                                    if ((p != null) && (p.isQoS())) {
                                        if (p.getRetryCount() >= QOS_TRY_COUNT) {
                                            if (ClientCoreSDK.DEBUG) {
                                                Log.e(QoS4SendDaemon.TAG, "【IMCORE】【QoS】指纹为" + p.getFp() + "的消息包重传次数已达" + p.getRetryCount() + "(最多" + QOS_TRY_COUNT + "次)上限，将判定为丢包！");
                                            }
                                            ...
                                            if (delta <= MESSAGES_JUST$NOW_TIME) {
                                                if (ClientCoreSDK.DEBUG) {
                                                    Log.e(QoS4SendDaemon.TAG, "【IMCORE】【QoS】指纹为" + key + "的包距\"刚刚\"发出才" + delta + "ms(<=" + MESSAGES_JUST$NOW_TIME + "ms将被认定是\"刚刚\"), 本次不需要重传哦.");
                                                }
                                            } else {
                                                new LocalUDPDataSender.SendCommonDataAsync(p) {
                                                    protected void onPostExecute(Integer code) {
                                                        if (code.intValue() == 0) {
                                                            ...
                                                            if (ClientCoreSDK.DEBUG)
                                                                Log.e(QoS4SendDaemon.TAG, "【IMCORE】【QoS】指纹为" + this.p.getFp() + "的消息包已成功进行重传，此次之后重传次数已达" + this.p.getRetryCount() + "(最多" + QOS_TRY_COUNT + "次).");
                                                        } else {
                                                            Log.e(QoS4SendDaemon.TAG, "【IMCORE】【QoS】指纹为" + this.p.getFp() + "的消息包重传失败，它的重传次数之前已累计为" + this.p.getRetryCount() + "(最多" + QOS_TRY_COUNT + "次).");
                                                        }
                                                    }
                                ...
                                Log.e(QoS4SendDaemon.TAG, "【IMCORE】【QoS】消息发送质量保证线程运行时发生异常," + eee.getMessage(), eee);
                            }
                        ...
                        protected void onPostExecute(ArrayList<Protocal> al) {
                            ...
                            QoS4SendDaemon.this.handler.postDelayed(QoS4SendDaemon.this.runnable, CHECH_INTERVAL);
                        }
                    }.execute(new Object[0]);
                }
            }
    ...
    public void put(Protocal p) {
        if (p == null) {
            Log.e(TAG, "Invalid arg p==null.");
            return;
        }
        if (p.getFp() == null) {
            Log.e(TAG, "Invalid arg p.getFp() == null.");
            return;
        }

        if (!p.isQoS()) {
            Log.e(TAG, "This protocal is not QoS pkg, ignore it!");
            return;
        }

        if (this.sentMessages.get(p.getFp()) != null) {
            Log.e(TAG, "【IMCORE】【QoS】指纹为" + p.getFp() + "的消息已经放入了发送质量保证队列，该消息为何会重复？（生成的指纹码重复？还是重复put？）");
        }
        ...
        new AsyncTask() {
            ...
            protected Object doInBackground(Object... params) {
                ...
                try {
                    ...
                    ret = sentMessages.remove(fingerPrint);
                } catch (Exception e) {
                    e.printStackTrace();
                }
                return ret;
            ...
            protected void onPostExecute(Object result) {
                Log.e(TAG, "【IMCORE】【QoS】指纹为" + fingerPrint + "的消息已成功从发送质量保证队列中移除(可能是收到接收方的应答也可能是达到了重传的次数上限)，重试次数=" + (result != null ? ((Protocal) result).getRetryCount() : "none呵呵."));
            }
        }.execute();
    }
