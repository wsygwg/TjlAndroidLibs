                    ...
                    new AsyncTask<Object, Integer, Integer>() {
                        protected Integer doInBackground(Object[] params) {
                            ...
                            if (ClientCoreSDK.autoReLogin) {
                                code = LocalUDPDataSender.getInstance(AutoReLoginDaemon.this.context).sendLogin(ClientCoreSDK.getInstance().getCurrentLoginName(), ClientCoreSDK.getInstance().getCurrentLoginPsw(), ClientCoreSDK.getInstance().getCurrentLoginExtra());
                            }
                        ...
                        protected void onPostExecute(Integer result) {
                            if (result.intValue() == 0) {
                                ...
                                //      当首次登陆后，因服务端或其它网络原因导致本地监听出错，将导致中断本地监听线程，所以在此处在自动登陆重连或用户自已手机尝试再次登陆时重启监听线程就可以恢复本地监听线程的运行。
