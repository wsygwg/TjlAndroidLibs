...
public class QoS4ReciveDaemon {
    ...
    public static final int CHECK_INTERVAL = 5 * 60 * 1000; //每隔5分钟做一次检查
    public static final int MESSAGES_VALID_TIME = 10 * 60 * 1000;     //消息有效时间：10分钟
    ...
    private QoS4ReciveDaemon(Context context) {
            ...
            public void run() {
                ...
                if (!QoS4ReciveDaemon.this._excuting) {
                    ...
                    if (ClientCoreSDK.DEBUG) {
                        Log.e(QoS4ReciveDaemon.TAG, "【IMCORE】【QoS接收方】++++++++++ START 暂存处理线程正在运行中，当前长度" + QoS4ReciveDaemon.this.recievedMessages.size() + ".");
                    }
                    for (String key : QoS4ReciveDaemon.this.recievedMessages.keySet()) {
                        ...
                        if (delta < MESSAGES_VALID_TIME)
                            continue;
                        if (ClientCoreSDK.DEBUG)
                            Log.e(QoS4ReciveDaemon.TAG, "【IMCORE】【QoS接收方】指纹为" + key + "的包已生存" + delta + "ms(最大允许" + MESSAGES_VALID_TIME + "ms), 马上将删除之.");
                        QoS4ReciveDaemon.this.recievedMessages.remove(key);
                    }
                }
                if (ClientCoreSDK.DEBUG) {
                    Log.e(QoS4ReciveDaemon.TAG, "【IMCORE】【QoS接收方】++++++++++ END 暂存处理线程正在运行中，当前长度" + QoS4ReciveDaemon.this.recievedMessages.size() + ".");
                }
                QoS4ReciveDaemon.this._excuting = false;
                QoS4ReciveDaemon.this.handler.postDelayed(QoS4ReciveDaemon.this.runnable, CHECK_INTERVAL);
            }
        };
    }
    ...
    public void addRecieved(String fingerPrintOfProtocal) {
        if (fingerPrintOfProtocal == null) {
            Log.e(TAG, "【IMCORE】无效的 fingerPrintOfProtocal==null!");
            return;
        }

        if (this.recievedMessages.containsKey(fingerPrintOfProtocal)) {
            Log.e(TAG, "【IMCORE】【QoS接收方】指纹为" + fingerPrintOfProtocal + "的消息已经存在于接收列表中，该消息重复了（原理可能是对方因未收到应答包而错误重传导致），更新收到时间戳.");
        }
