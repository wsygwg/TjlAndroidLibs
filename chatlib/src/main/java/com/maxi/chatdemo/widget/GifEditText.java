...
public class GifEditText extends EditText {
    ...
    TextWatcher tw = new TextWatcher() {
        ...
        public void onTextChanged(CharSequence s, int start, int before, int count) {
            ...
            Integer faceId = null;
            if((faceId = FaceData.gifFaceInfo.get(s)) != null){
                Log.e(TAG,"添加了表情");
            }
            if(count != before){
                changeNum = count - before;
                twPositionAfter = start + changeNum;
                Log.e(TAG,"####################changeNum = " + changeNum + " ; twPositionAfter = " + twPositionAfter);
            }
        }
        ...
        public void afterTextChanged(Editable s) {
            ...
            if (!temp.toString().equals(s.toString())) {
                temp = s.toString();
                setSpanText(handler, getText().toString(), true);
                Log.e(TAG, "temp after = " + temp);
            } else {
                Log.e(TAG, "temp is not changed = " + temp);
            }
        }
    };
...
//    }
    ...
    public boolean parseMessage(GifEditText gifTextView) {
        if (gifTextView.myText != null && !gifTextView.myText.equals("")) {
            ...
            for (int i = 0; i < gifTextView.spanInfoList.size(); i++) { // for循环，处理显示多个图片的问题
                ...
                /**currentFrameIndex,用于控制当前应该显示的帧的序号，每次显示之后currentFrameIndex 应该加1，加到frameCount后再变成0循环显示*/
                int size = ScreenUtil.dip2px(gifTextView.getContext(), 18);
                if (gifCount != 0) {
            ...
            gifTextView.setSelection(twPositionAfter);
    ...
    private TextRunnable rTextRunnable;
    ...
    public class TextRunnable implements Runnable {
        private GifEditText gifTextView;

        public TextRunnable(GifEditText gifTextView) {
            this.gifTextView = gifTextView;
        }

        @Override
        public void run() {
            if (gifTextView != null && runFlag) {
                /**
                 * 这一步是为了节省内存而是用，即如果文本中只有静态图片没有动态图片，那么该线程就此终止，不会重复执行。而如果有动图，那么会一直执行
                 */
