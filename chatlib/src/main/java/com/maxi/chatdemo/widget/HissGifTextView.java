...
public class HissGifTextView extends TextView {
    /**
     ...
     */
    @SuppressLint("NewApi")
    public HissGifTextView(Context context, AttributeSet attrs, int defStyle) {
        super(context, attrs, defStyle);
        // spanInfoList = new ArrayList<SpanInfo>();
//        HissGifTextView.this.setFocusableInTouchMode(false);
    }

    @SuppressLint("NewApi")
    public HissGifTextView(Context context, AttributeSet attrs) {
        super(context, attrs);
        // spanInfoList = new ArrayList<SpanInfo>();
//        HissGifTextView.this.setFocusableInTouchMode(false);
    }
    ...
    @SuppressLint("NewApi")
    public HissGifTextView(Context context) {
        super(context);
        // spanInfoList = new ArrayList<SpanInfo>();
//        HissGifTextView.this.setFocusableInTouchMode(false);
    }
    ...
    public boolean parseMessage(HissGifTextView gifTextView) {
        if (gifTextView.myText != null && !gifTextView.myText.equals("")) {
            ...
            for (int i = 0; i < gifTextView.spanInfoList.size(); i++) { // for循环，处理显示多个图片的问题
                ...
                int size = ScreenUtil.dip2px(gifTextView.getContext(), 13);
                if (gifCount != 0) {
    ...
    public static final class TextRunnable implements Runnable {
        private final WeakReference<HissGifTextView> mWeakReference;

        public TextRunnable(HissGifTextView f) {
            mWeakReference = new WeakReference<HissGifTextView>(f);
        }

        @Override
        public void run() {
            HissGifTextView gifTextView = mWeakReference.get();
            if (gifTextView != null) {
                /**
