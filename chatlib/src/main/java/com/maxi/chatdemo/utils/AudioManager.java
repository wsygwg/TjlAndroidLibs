...
public class AudioManager {
    ...
    public static String getAudioSavePath(){
        return FileSaveUtil.FILE_BASE_PATH + CacheData.getMyData().getMemberId() + FileSaveUtil.IM_FILE_PATH;
    }
...
//    }
    ...
    public void prepareAudio() {
        try {
            Log.e(TAG,"prepareAudio start at " + HissDate.getCurrentDate(HissDate.DATE_FORMAT_YMDHMS_DEFAULT));
            // 一开始应该是false的
            isPrepared = false;

            File dir = new File(getAudioSavePath());
            if (!dir.exists()) {
                dir.mkdirs();
            }
            ...
            myAudioRecorder = new MyAudioRecorder();
            myAudioRecorder.prepare();
            Log.e(TAG,"prepareAudio finished at " + HissDate.getCurrentDate(HissDate.DATE_FORMAT_YMDHMS_DEFAULT));
            // 准备结束
            // 已经准备好了，可以录制了
            if (mListener != null) {
    ...
    public int getVoiceLevel(int maxLevel) {
        ...
        if (isPrepared) {
            try {
                ...
                return (int) myAudioRecorder.getdBvolume() / 20;
            } catch (Exception e) {
                if (handler != null)
                    handler.sendEmptyMessage(MSG_ERROR_AUDIO_RECORD);
            }
    ...
    public void release() {
        if(myAudioRecorder != null && mCurrentFilePathString != null){
            myAudioRecorder.stopRecording(mCurrentFilePathString);
